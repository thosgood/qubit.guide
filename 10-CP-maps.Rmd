...
$$
  ...
  \\\mbox{where $\sum_i A^\dagger_i A_i=\id$}
  \end{gathered}
$$
and
$$
  ...
  \\\mbox{where $\sum_j B^\dagger_j B_j=\id$}
  \end{gathered}
$$
then
$$
...
$$
  ...
  \quad\mbox{with $U^\dagger U=\id$}
$$
to
$$
  \rho
  \longmapsto \sum_k E_k\rho E^\dagger_k
  \quad\mbox{with $\sum_k E^\dagger_k E_k=\id$.}
$$
...
(ref:cp-test-caption) Test for complete positivity: apply the map $\mathcal{E}$ to half of a maximally entangled state and check if the resulting bipartite operator is a density operator. **!!!TODO!!! add something like "is this a density operator?" in an `rstick` in the diagram**
...
```{r,engine='tikz',engine.opts=list(template="tikz2pdf.tex"),fig.width=14}
\newcommand{\id}{\mathbf{1}}
\begin{equation*}
  \begin{quantikz}
    &&\qw &\gate{\id} &\qw &\qw
...
**!!!TODO!!! caption the above diagram! be clear with what it really means (so probably draw some arrows)**
...
**!!!TODO!!! this diagram seems confusing given the previous one: the three circuits aren't in bijection!**

**!!!TODO!!! caption the above diagram as well! are we saying everything is equal?**
