...
(ref:environment-same-channel-caption) Here the $\times$ represents discarding the final state of the environment.
...
\begin{equation*}
  ...
  \\\lstick{$\ket{e}$} &\hphantom{really wide} &\qw &\qw &\rstick{$\times$} \qw
  \end{quantikz}
\end{equation*}
\begin{equation*}
  \begin{quantikz}
    \lstick{$\ket{\psi}$} & \gate[wires=2]{U} &\qw &\rstick{$\rho'$} \qw
  \\\lstick{$\ket{e}$} &\hphantom{really wide} &\gate{R} &\rstick{$\times$} \qw
  \end{quantikz}
\end{equation*}
```
