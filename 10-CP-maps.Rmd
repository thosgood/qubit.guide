...
$$
  \mathcal{B}(\mathcal{H}_{\mathcal{A}})
  \longrightarrow \mathcal{B}(\mathcal{H}_{\mathcal{A}}\otimes\mathcal{H}_{\mathcal{B}}).
$$
Conversely, given a composite system $\mathcal{AB}$, we can discard subsystem $\mathcal{B}$, which is described by a map (**reduction**)
$$
  \mathcal{B}(\mathcal{H}_{\mathcal{A}}\otimes\mathcal{H}_{\mathcal{B}})
  \longrightarrow \mathcal{B}(\mathcal{H}_{\mathcal{A}}).
$$
...
::: {.idea data-latex=""}
A linear map $\mathcal{E}\colon B(\mathcal{H}) \to B(\mathcal{H}')$ is completely positive iff $(\mathcal{I}\otimes\mathcal{E})(\proj{\Omega})\geq0$, where $\ket{\Omega}$ is a maximally entangled state of dimension $d=\dim{\mathcal{H}}$.
:::
...
If we choose bases of $\mathcal{H}$ and $\mathcal{H}'$ (where $\dim\mathcal{H}=d$ and $\dim\mathcal{H}'=d'$), then any linear map $\mathcal{E}\colon\mathcal{B}(\mathcal{H})\to\mathcal{B}(\mathcal{H'})$ can be completely characterised by its action on the $d^2$ basis matrices $\ket{i}\bra{j}$, where $i,j=1,2\ldots,d$.
...
$$
**!!!TODO!!! add some brackets here**
:::

For example, if we are interested in the component $\mathcal{E}(X)_{ij}= \bra{i}\mathcal{E}(X)\ket{j}$, then we take $Y=\ket{j}\bra{i}$.

The Choi matrix $\widetilde{\mathcal{E}}$ in $\mathcal{B}(\mathcal{H}\otimes\mathcal{H'})$ is essentially another way of representing the linear map $\mathcal{E}\colon\mathcal{B}(\mathcal{H})\to\mathcal{B}(\mathcal{H'})$.
One can express all properties of $\mathcal{E}$ in terms of properties of $\widetilde{\mathcal{E}}$, e.g. "$\mathcal{E}$ is completely positive if and only if $\widetilde{\mathcal{E}}\geq0$", or "$\mathcal{E}$ is trace preserving if $\tr_{\mathcal{B}}\widetilde{\mathcal{E}}=d\id$".
...
As a closing remark before moving on, note that the equivalence of linear maps $\mathcal{B}(\mathcal{H})\to \mathcal{B}(\mathcal{H'})$ and matrices in $\mathcal{B}(\mathcal{H}\otimes\mathcal{H'})$ is well known.
...
::: {.idea data-latex=""}
A linear map $\mathcal{E}\colon\mathcal{B}(\mathcal{H})\to\mathcal{B}(\mathcal{H}')$ is completely positive iff it admits an operator sum (Kraus) decomposition of the form
$$
...
- For _any_ $\mathcal{E}\colon\mathcal{B}(\mathcal{H})\to\mathcal{B}(\mathcal{H}')$, there is _always_ a representation with at most $dd'$ mutually orthogonal Kraus operators: $\tr E_i^\dagger E_j\propto\delta_{ij}$.
:::
...
The correspondence between linear maps $\mathcal{B}(\mathcal{H})\to\mathcal{B}(\mathcal{H'})$ and operators in $\mathcal{B}(\mathcal{H}\otimes\mathcal{H'})$, known as the Choi-Jamiołkowski isomorphism, is another example of a well known correspondence between vectors in $\mathcal{H}_{\mathcal{A}}\otimes\mathcal{H}_{\mathcal{B}}$ and operators $\mathcal{B}(\mathcal{H}_{\mathcal{A}}^\star,\mathcal{H}_{\mathcal{B}})$ or $\mathcal{B}(\mathcal{H}_{\mathcal{B}}^\star,\mathcal{H}_{\mathcal{A}})$.

Take a tensor product vector in $\ket{a}\otimes\ket{b}\in \mathcal{H}_{\mathcal{A}}\otimes\mathcal{H}_{\mathcal{B}}$.
Then it defines natural maps in $\mathcal{B}(\mathcal{H}_{\mathcal{A}}^\star,\mathcal{H}_{\mathcal{B}})$ and $\mathcal{B}(\mathcal{H}_{\mathcal{B}}^\star,\mathcal{H}_{\mathcal{A}})$, via
$$
  \begin{aligned}
    \bra{x}
...
However, some care must be taken when we want to define correspondence between vectors in $\mathcal{H}_{\mathcal{A}}\otimes\mathcal{H}_{\mathcal{B}}$ and operators in $\mathcal{B}(\mathcal{H}_{\mathcal{A}},\mathcal{H}_{\mathcal{B}})$ or $\mathcal{B}(\mathcal{H}_{\mathcal{B}},\mathcal{H}_{\mathcal{A}})$.
For example, physicists like to "construct" $\mathcal{B}(\mathcal{H}_{\mathcal{B}},\mathcal{H}_{\mathcal{A}})$ in a deceptively simple way:
$$
  \ket{a}\ket{b} \leftrightarrow \ket{a}\bra{b}.
$$
...
$$
  \ket{i}\ket{b} \leftrightarrow \sum_j \beta_j \ket{i}\bra{j}
$$
and _not_ like
$$
  \ket{i}\ket{b} \leftrightarrow \ket{i}\bra{b}
...
- If we take the transposition in the Schmidt basis of $\ket{\Omega}$, then
    $$
...
- Any pure state of the bipartite system $\ket{\psi}=\sum_{ij} c_{ij}\ket{i}\ket{j}$ can be written as
    ...
    $$
    (where $\overline{U}$ denotes the matrix given by taking the complex conjugate, entry-wise, of $U$, i.e. _without_ taking the transpose as well).

- The swap operation, $S\colon\ket{i}\ket{j}\mapsto\ket{j}\ket{i}$, can be expressed as
    $$
      \begin{aligned}
        S
        &= d \proj{\Omega}^{T_A}
...
```{r visualising-partial-trace,engine='tikz',fig.width=8,fig.cap="Visualising the two partial traces of a matrix _written in the tensor product basis_."}
\begin{equation*}
  \begin{array}{c}
    \left[
      \,
