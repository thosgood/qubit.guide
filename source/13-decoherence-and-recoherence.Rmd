...
\begin{tikzpicture}
  ...
  \node [draw,primary,minimum width=1cm,thick,minimum height=1cm,rotate=-10] at (4,0.3) (b) {$\mathcal{C}_2$};
  \node [draw,primary,minimum width=1cm,thick,minimum height=1cm,rotate=25] at (5.5,-0.5) (c) {$\mathcal{C}_3$};
  \draw [->,thick] (start) to[out=60,in=150] node [midway,above] {$E_1$} (a);
  \draw [->,thick] (start) to[out=20,in=150] node [pos=0.35,above] {$E_2$} (b);
  \draw [->,thick] (start) to[out=-60,in=-1,loop] node [midway,right] {$E_0$} (start);
  \draw [->,thick] (start) to[out=-80,in=-160] node [midway,above] {$E_3$} (c);
  \node[draw=secondary,inner sep=15pt,thick,fit=(start) (a) (b) (c),label={[text=secondary]above:$\mathcal{H}'$}] {};
\end{tikzpicture}
```
...
## Discretisation of quantum errors {#discretisation-of-quantum-errors}
...
Assuming that the errors are independent^[The assumption that the errors are independent is very important, but not always physically realistic! We will re-examine to this assumption a few times in Chapter \@ref(error-correction).] then, for any given triplet,
$$
  \begin{cases}
    \text{no errors}
...
(ref:error-probability-for-000-caption) What can happen, and the respective probabilities, when we transmit the codeword $000$. With this scheme, we can correct up to one error, and detect up to two. Note that when two errors occur, the "majority vote" correction scheme actually gives the *wrong* "correction".

```{r error-probability-for-000,engine='tikz',fig.cap='(ref:error-probability-for-000-caption)'}
\usetikzlibrary{decorations.markings,decorations.pathmorphing,decorations.pathreplacing}
\begin{tikzpicture}[xscale=0.4,yscale=0.5]
  ...
  \begin{scope}[shift={(22,0)}]
    ...
    \node[align=right] (one) at (-0.5,6) [left] {one error\\[-0.3em]\textit{correctable}};
    \node[align=right] (two) at (-0.5,3) [left] {two errors\\[-0.3em]\textit{detectable}};
    \node[align=right] (three) at (-0.5,1) [left] {three errors\\[-0.3em]\textit{not detectable}};
  \end{scope}
\end{tikzpicture}
```
...
This is known as **fault tolerance**, and studying this, using the stabiliser formalisation of Chapter \@ref(stabilisers), is the goal of Chapter \@ref(error-correction).
