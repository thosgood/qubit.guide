# Decoherence and recoherence {#decoherence-and-recoherence}
...
> But also about how we can start to deal with it via some elementary **error correction**, including the **Shor $[[9,1,3]]$ quantum code**, which generalises the classical **three-bit repetition code**.
...
To counteract this problem, we will start to look at the idea of **error correcting codes**, which protect our data against unwanted errors, but at the cost of encoding it across more ancillary qubits.





## The three-qubit code {#three-qubit-code}
...
\begin{tikzpicture}
  \begin{scope}[shift={(-9,0)}] % the figure on the left-hand side
    \node [draw,primary,minimum width=1cm,thick,minimum height=1cm] at (0,0.75) (start) {$\mathcal{H}$};
    \node [draw,primary,minimum width=1cm,thick,minimum height=1cm,rotate=15] at (4,2) (a) {$\mathcal{H}_1$};
    \node [draw,primary,minimum width=1cm,thick,minimum height=1cm,rotate=0] at (5.5,1.5) (b) {$\mathcal{H}_2$};
    \node [draw,primary,minimum width=1cm,thick,minimum height=1cm,rotate=-10] at (4,0) (c) {$\mathcal{H}_3$};
    \node [draw,primary,minimum width=1cm,thick,minimum height=1cm,rotate=25] at (5.5,-0.5) (d) {$\mathcal{H}_4$};
    \draw [->,thick] (start) to[out=60,in=150] node [midway,above] {$V_1$} (a);
    \draw [->,thick] (start) to[out=20,in=-150] node [pos=0.35,above] {$V_2$} (b);
    \draw [->,thick] (start) to[out=-60,in=-170] node [midway,above] {$V_3$} (c);
    \draw [->,thick] (start) to[out=-80,in=-160] node [midway,above] {$V_4$} (d);
    \node[draw=secondary,inner sep=15pt,thick,fit=(a) (b) (c) (d),label={[text=secondary]above:$\mathcal{H}'$}] {};
  \end{scope}
  \begin{scope} % the figure on the right-hand side
    \begin{scope} % plot the intersections first
      ...
      \pgfset{minimum width=1cm,minimum height=1cm}
      \pgftransformshift{\pgfpoint{4cm}{2cm}}
      \pgftransformrotate{15}
      \pgfnode{rectangle}{center}{}{nodename}{\pgfusepath{stroke,clip}}
      \pgftransformreset
      \node [draw,fill=primary, minimum width=1cm,thick,minimum height=1cm,rotate=0] at (4.85,1.5) {};
    \end{scope}
    \begin{scope}
      %\pgfset{shape aspect=0.5} Uncomment this and remove minimum size for this option
      \pgfset{minimum width=1cm,minimum height=1cm}
      \pgftransformshift{\pgfpoint{4.5cm}{0cm}}
      \pgftransformrotate{-10}
      \pgfnode{rectangle}{center}{}{nodename}{\pgfusepath{stroke,clip}}
      \pgftransformreset
      \node [draw,fill=primary, minimum width=1cm,thick,minimum height=1cm,rotate=25] at (5.25,-0.5) {};
    \end{scope}
    \node [draw,primary,minimum width=1cm,thick,minimum height=1cm] at (0,0.75) (start) {$\mathcal{H}$};
    \node [draw,primary,minimum width=1cm,thick,minimum height=1cm,rotate=15] at (4,2) (a) {$\mathcal{H}_1$};
    \node [draw,primary,minimum width=1cm,thick,minimum height=1cm,rotate=0] at (4.85,1.5) (b) {$\mathcal{H}_2$};
    \node [draw,primary,minimum width=1cm,thick,minimum height=1cm,rotate=-10] at (4.5,0) (c) {$\mathcal{H}_3$};
    \node [draw,primary,minimum width=1cm,thick,minimum height=1cm,rotate=25] at (5.25,-0.5) (d) {$\mathcal{H}_4$};
    \draw [->,thick] (start) to[out=60,in=150] node [midway,above] {$V_1$} (a);
...
Here is a simple but important example: the **three-qubit code**.^[We will return to this example, using the language of stabilisers from Chapter \@ref(stabilisers), in Chapter \@ref(error-correction).]
Take a qubit in some pure state $\ket{\psi}=\alpha\ket{0}+\beta\ket{1}$, introduce two auxiliary qubits in a fixed state $\ket{0}\ket{0}$, and apply a unitary operation to the three qubits, namely two controlled-$\NOT$ gates:
...
In other words, rather than thinking of Alice as picking randomly between four different isometries, we can imagine that she *always* picks the **encoding** isometry $V_{00}$, and then some noisy process randomly applies one of the four actions $\id$, $\id\id X$, $\id X\id$, or $X\id\id$.
...
\usetikzlibrary{decorations,shapes,fit}
\begin{tikzpicture}
  \node [draw,secondary,minimum width=1cm,thick,minimum height=1cm] at (-4,0.75) (origin) {$\mathcal{H}$};
  \draw [->,thick] (origin) to node [midway,above] {encode} (start);
  \node [draw,primary,minimum width=1cm,thick,minimum height=1cm] at (0,0.75) (start) {$\mathcal{C}$};
  \node [draw,primary,minimum width=1cm,thick,minimum height=1cm,rotate=15] at (4,2) (a) {$\mathcal{C}_1$};
  %\node [draw,primary,minimum width=1cm,thick,minimum height=1cm,rotate=0] at (5.5,1.5) (b) {$\mathcal{C}_2$};
  \node [draw,primary,minimum width=1cm,thick,minimum height=1cm,rotate=-10] at (4,0.3) (c) {$\mathcal{C}_2$};
  \node [draw,primary,minimum width=1cm,thick,minimum height=1cm,rotate=25] at (5.5,-0.5) (d) {$\mathcal{C}_3$};
  \draw [->,thick] (start) to[out=60,in=150] node [midway,above] {$E_1$} (a);
  \draw [->,thick] (start) to[out=20,in=150] node [pos=0.35,above] {$E_2$} (c);
  \draw [->,thick] (start) to[out=-60,in=-1,loop] node [midway,right] {$E_0$} (c);
  \draw [->,thick] (start) to[out=-80,in=-160] node [midway,above] {$E_3$} (d);
  \node[draw=secondary,inner sep=15pt,thick,fit=(start) (a) (b) (c) (d),label={[text=secondary]above:$\mathcal{H}'$}] {};
\end{tikzpicture}
```
...
$$
  V_x = U_xV_0
$$
for some set of unitaries $\{U_x\}$ with $U_0=\id$, then an **encoding** operation $V_0$ provides protection against the **errors** $U_x$.
:::
...
```{r,engine='tikz'}
  \begin{tikzpicture}[scale=0.5]
    ...
    \draw [process] (initial) to [out=50,in=120]  (final1);
    \draw [process] (initial) to [out=-70,in=-150] (final2);
  \end{tikzpicture}
```
...
$$
  \begin{aligned}
    \ket{O_1}
    = \ket{O}\ket{e_1}
    &= \text{``computer shows output $O$, the environment knows that path 1 was taken''}
    \ket{O_2}
    = \ket{O}\ket{e_2}
    &= \text{``computer shows output $O$, the environment knows that path 2 was taken''}
  \end{aligned}
$$

The two final states $O_1$ and $O_2$ are identical if and only if $|\braket{e_1}{e_1}|=1$.
...
In contrast, the two final states $O_1$ and $O_2$ are completely different if and only if $|\braket{e_1}{e_2}|=0$.
Then there is only one path to the output --- there is no quantum interference --- and there is nothing to add.
Of course, there are also midway cases $0<|\braket{e_1}{e_2}|<1$ corresponding to partial distinguishability of the final states.
...
It involves the environment gradually acquiring information about computational paths and the associated relative environmental states ($\ket{e_0}$ and $\ket{e_1}$ in our example above), which evolve over time to become increasingly orthogonal to one another.
...
## Digitising quantum errors {#digitising-quantum-errors}
...
In particular, this channel turns the pure state $\ket{\psi}$ into a statistical mixture of states $\ket{\widetilde{\psi_i}}=E_i\ket{\psi}$.
Note that the $\ket{\widetilde{\psi_i}}$ are not normalised:
$$
  p_i
  \coloneqq \braket{\widetilde{\psi_i}}{\widetilde{\psi_i}}
  = \bra{\psi}E_i^\dagger E_i\ket{\psi}
$$
is exactly the probability with which the normalised version of $E_i\ket{\psi}$ appears in the mixture.
This mixture may arise if one measures the environment in the $\ket{e_i}$ basis and then forgets about the result.





## When can recoherence work?

If we could measure the environment in the $\ket{e_i}$ basis without forgetting the result, then we would know what kind of error had occurred (say, some $E_k$) and could then simply restore the original state $\ket{\psi}$ by reversing the action of $E_k$.
...
For example, in Section \@ref(towards-error-correction) we saw how the three-qubit encoding could correct for one of the possible errors^[Quite often we will write $X_i$ to mean "an $X$ error on the $i$-th qubit", so that e.g. $X_2=\id\otimes X_2\otimes\id\ldots\otimes\id$.] $\id$, $X_1$, $X_2$, or $X_3$.
...
$$
  \ket{\psi}
  \xmapsto{R_{X_1}(\theta)} \cos\frac{\theta}{2}\ket{\psi} + i\sin\frac{\theta}{2}X_1\ket{\psi}.
$$
...
$$
where $\binom{m}{n}=m!/(n!(m-n)!)$ is the [**binomial coefficient**](https://en.wikipedia.org/wiki/Binomial_coefficient).
...
$$
i.e. when $p<1/2$.
This important number is known as the **error threshold** of the code, and is a good judge of how useful a code actually is.
...
(ref:error-probability-for-000-caption) What can happen, and the respective probability, when we transmit the codeword $000$. With this scheme, we can correct up to one error, and detect up to two. Note that when two errors occur, the "majority vote" correction scheme actually gives the *wrong* "correction".
...
\begin{tikzpicture}[xscale=0.4,yscale=0.5]
  \begin{scope}
    \node (0) at (1,8) [fill=gray!20] {$000$};
    \node (000) at (10,8) [fill=green!20] {$000$};
    \node (001) at (10,7) [fill=yellow!20] {$001$};
    \node (010) at (10,6) [fill=yellow!20] {$010$};
    \node (100) at (10,5) [fill=yellow!20] {$100$};
    \node (110) at (10,4) [fill=orange!40] {$110$};
    \node (101) at (10,3) [fill=orange!40] {$101$};
    \node (011) at (10,2) [fill=orange!40] {$011$};
    \node (111) at (10,1) [fill=red!20] {$111$};
    \draw[->,thick] (0) to (000.west);
    \draw[->,thick] (0) to (001.west);
    \draw[->,thick] (0) to (010.west);
    \draw[->,thick] (0) to (100.west);
    \draw[->,thick] (0) to (110.west);
    \draw[->,thick] (0) to (101.west);
    \draw[->,thick] (0) to (011.west);
    \draw[->,thick] (0) to (111.west);
    \draw [decorate,decoration={brace,amplitude=5pt,raise=2pt}] (000.north east) to (000.south east);
    \draw [decorate,decoration={brace,amplitude=5pt,raise=2pt}] (001.north east) to (100.south east);
    \draw [decorate,decoration={brace,amplitude=5pt,raise=2pt}] (110.north east) to (011.south east);
    \draw [decorate,decoration={brace,amplitude=5pt,raise=2pt}] (111.north east) to (111.south east);
    \node (noer) at (11.5,8) [right] {$(1-p)^{3}$};
    \node (one) at (11.5,6) [right] {$3p(1-p)^{2}$};
    \node (two) at (11.5,3) [right] {$3p^{2}(1-p)$};
    \node (three) at (11.5,1) [right] {$p^{3}$};
  \end{scope}
  \begin{scope}[shift={(22,0)}]
    \node (0) at (10,8) [fill=gray!20] {$000$};
    \node (1) at (10,1) [fill=gray!20] {$111$};
    \node (000) at (1,8) [fill=green!20] {$000$ };
    \node (001) at (1,7) [fill=yellow!20] {$001$};
    \node (010) at (1,6) [fill=yellow!20] {$010$};
    \node (100) at (1,5) [fill=yellow!20] {$100$};
    \node (110) at (1,4) [fill=orange!40] {$110$};
    \node (101) at (1,3) [fill=orange!40] {$101$};
    \node (011) at (1,2) [fill=orange!40] {$011$};
    \node (111) at (1,1) [fill=red!20] {$111$};
    \draw[->,thick] (000.east) to (0);
    \draw[->,thick] (001.east) to (0);
    \draw[->,thick] (010.east) to (0);
    \draw[->,thick] (100.east) to (0);
    \draw[->,thick] (110.east) to (1);
    \draw[->,thick] (101.east) to (1);
    \draw[->,thick] (011.east) to (1);
    \draw[->,thick] (111.east) to (1);
    \draw [decorate,decoration={brace,amplitude=5pt,raise=2pt}] (000.south west) to (000.north west);
    \draw [decorate,decoration={brace,amplitude=5pt,raise=2pt}] (100.south west) to (001.north west);
    \draw [decorate,decoration={brace,amplitude=5pt,raise=2pt}] (011.south west) to (110.north west);
    \draw [decorate,decoration={brace,amplitude=5pt,raise=2pt}] (111.south west) to (111.north west);
    \node (noer) at (-0.5,8) [left] {no error};
    \node[align=right] (one) at (-0.5,6) [left] {one error\\[-0.4em]\textit{correctable}};
    \node[align=right] (two) at (-0.5,3) [left] {two errors\\[-0.4em]\textit{detectable}};
    \node[align=right] (three) at (-0.5,1) [left] {three errors\\[-0.4em]\textit{not detectable}};
  \end{scope}
\end{tikzpicture}
```
...
We can't be certain *which* error has occurred, since it could have been a single bit-flip on $111$ (more likely) or a double bit-flip on $000$ (less likely), but we know for sure that *one of these two* went wrong.
...
This results in what is known as a **logical error**, where the corrupted string is also a codeword, leaving us with no indication that anything has gone wrong.
...
More generally, the distance $d$ relates to the number of errors $t$ that a code can correct by
$$
  d = 2t+1
$$
...
$$
  \alpha\ket{0}+\beta\ket{1}
  \longmapsto (\alpha\ket{0}+\beta\ket{1})(\alpha\ket{0}+\beta\ket{1})(\alpha\ket{0}+\beta\ket{1}).
$$
Rather than repeating the qubit like this, the three-qubit code sort of "smears it out" across three qubits, resulting in the *entangled* state $\alpha\ket{000}+\beta\ket{111}$.
...
To detect errors in a repetition encoding, we consider the parity of *all* pairs of qubits in the code; in the case of the three-qubit repetition code, we use the operators
$$
  \begin{gathered}
    Z\otimes Z\otimes\id
    Z\otimes \id\otimes Z
    \id\otimes Z\otimes Z.
  \end{gathered}
$$
However, since $Z^2=\id$, it actually suffices to use only two of these, say $Z\otimes Z\otimes\id$ and $\id\otimes Z\otimes Z$, because we can recover the last one as their product.
But these two operators are exactly generators of the stabiliser group
$$
  \mathcal{S}
  = \{\id\id\id, ZZ\id, Z\id Z, \id ZZ\}
$$
...
Note how the $+$ and $-$ results correspond to the binary labels $0$ and $1$, and also how the $Z\id Z$ column is simply given by the product of the other two measurement columns.
...
If we write $\overline{x}$ to mean the complement $\NOT(x)$ of a binary string $x$, then these can all be expressed as
$$
  V_x
...
(ref:the-shor-913-code-encoding-caption) The encoding circuit for the Shor $[[9,1,3]]$-code, implementing $(V_{00}\otimes V_{00}\otimes V_{00})(H\otimes H\otimes H)V_{00}$. The three locations marked with numbers are not part of the circuit, but we will use them to explain how this circuit corrects for arbitrary single-qubit errors.

```{r the-shor-913-code-encoding,engine='tikz',engine.opts=list(template="latex/tikz2pdf.tex"),fig.width=4.5,fig.cap='(ref:the-shor-913-code-encoding-caption)'}
\definecolor{secondary}{RGB}{91,132,177}
...
We will describe how the Shor $[[9,1,3]]$-code^[We use double square brackets to emphasise that this is a quantum, not classical, code.] provides single-qubit error correction in more detail in Section \@ref(the-shor-913-code).
...
## Correcting any single error: Shor [[9,1,3]] {#the-shor-913-code}
...
Trying to answer questions like this leads us to [**operads**](https://en.wikipedia.org/wiki/Operad) and their algebras.
...
You might find some intriguing pictures if you search for the **little cubes operad**, or the **Swiss cheese operad**.
One particularly nice introduction is Tai-Danae Bradley's ["What is an Operad?"](https://www.math3ma.com/blog/what-is-an-operad-part-1).
:::
...
In fact, this scheme can tolerate up to *three* bit-flip errors *provided that they occur in different blocks*.^[We will explain why this condition of "occurring in different blocks" is necessary in Section \@ref(error-correcting-conditions).]
...
This is known as **fault tolerance**, and studying this, using the stabiliser formalisation of Section \@ref(stabilisers), is the goal of Section \@ref(error-correction).
...
Before we can describe fully fault-tolerant computation, we need to be able to deal with these more complicated scenarios, and this forms the topic of a large chunk of Section \@ref(error-correction).

However, there are other assumptions that we are making that we will *not* discuss in this text, because they fall out of the scope of "introductory" and instead become the topic of specialised research in error correction and fault tolerance.
...
Another assumption is something more "implementation-focused", namely that we are correctly operating the measurement devices, and that they are well-calibrated, otherwise we run into the problem of **measurement errors**.
Often combined with this is the problem of **state preparation**^[The combination of state preparation and measurement errors is sometimes called **SPAM**. This can be dealt with in various ways, appealing to the fact that their effect does not get worse as circuit depth increases since these problems are located at the very beginning and very end of the circuit.] --- how do we know that we really are preparing the state that we call $\ket{0}$?
...
3. $\{U^{\otimes4}V_0 \mid U\text{ unitary}\}$, where
  $$
    \begin{aligned}
      V_0
...
### More on Shor [[9,1,3]] {#more-on-shor-913}
...
3. Can you identify the stabilisers and the **logical operators** $X_L$ and $Z_L$ for this code?^[That is, the operators $X_L$ and $Z_L$ that behave on $\ket{0}_L$ and $\ket{1}_L$ exactly how $X$ and $Z$ behave on $\ket{0}$ and $\ket{1}$. *Hint: start from the encoding circuit with the eight ancillas all prepared in state $\ket{0}$; what are their stabilisers? Recall that the encoding operation is a Clifford circuit.*]
