...
::: {.idea latex=""}
Apart from individual unitaries or isometries, the only **correctable** channels are exactly the mixtures of $\{V_i\}$ such that $V^\dagger_i V_j=\delta_{ij}\id$, i.e. mixtures of mutually orthogonal isometries.
:::
...
We can draw this scenario using a diagram that that we have already seen:^[In this section we will be using and building upon the theory of stabilisers from Section \@ref(stabilisers) quite a lot.]
...
$$
  ...
  = \proj{001} + \proj{101}
$$
will then detect $\mathcal{H}'_2$ as the subspace where the output state resides, but the measurement (i.e. the corresponding projection) will not affect any state in that subspace.
...
This, however, *should not be taken literally* unless the states of the environment, $\ket{e_{\mathbf{1}}}$ and $\ket{e_Z}$, are orthogonal.^[**Exercise.** Why not?]
