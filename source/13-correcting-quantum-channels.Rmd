...
## Protecting against bit-flips {#protecting-against-bit-flips}
...
## Protecting against phase-flips {#protecting-against-phase-flips}
...
Having dealt with the first, we now deal with the second; finding a way to combine these two solutions to deal with the third is the subject of Section \@ref(protecting-against-bit-phase-flips).
...
But just as how the circuit in Section \@ref(protecting-against-bit-flips) only protected against bit-flips, this circuit only protects against phase-flips --- now we need to find a way to combine them.
...
We will describe how the Shor $[[9,1,3]]$-code^[We use double square brackets to emphasise that this is a quantum, not classical, code.] provides single-qubit error correction in more detail in Section \@ref(protecting-against-bit-phase-flips).
...
## Protecting against bit-phase-flips {#protecting-against-bit-phase-flips}

In Section \@ref(composing-correctable-channels) we derived the encoding circuit for the Shor $[[9,1,3]]$-code, so now let's put all the pieces together and understand how this gives an error correction procedure for all possible single-qubit errors.
...
Rather than drawing the entire circuits from Sections \@ref(protecting-against-bit-flips) and \@ref(protecting-against-phase-flips) again, let's simply draw them as consisting of an encoding gate $C$ and a recovery gate $R$, separated by a zone $[t_1,t_2]$ that corresponds to transmission over the noisy channel.
...
and the phase-flip correction looks the same, but with Hadamards sandwiching the transmission zone, so
...
\begin{quantikz}[row sep=tiny]
  ...
  & \gate[wires=3]{R}
  & \gate{H}
  & \gate[wires=9,nwires={2,3,5,6,8,9}]{R}
  & \qw \rstick{$\ket{\psi}$}
\\[-1em]
  &&&& \qw
  & \qw
