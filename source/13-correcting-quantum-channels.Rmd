...
> But also about how we can start to deal with it via some elementary **error correction**, including the **Shor [[9,1,3]] quantum code**.
...
If $\mathcal{H}'$ is "sufficiently bigger" than $\mathcal{H}$, and if the images $\mathcal{H}'_i\coloneqq V_i(\mathcal{H})$ do not overlap^[More precisely, we say that the $\mathcal{H}'_i$ "do not overlap" to mean that the subspaces $\mathcal{H}'_i$ are mutually orthogonal] then we can reverse the action of the channel:
...
For example, if Alice randomly picked $V_{01}$, then the input state $\ket{\psi}=\alpha_0\ket{0}+\alpha_1\ket{1}$ will be mapped to the output state $\alpha_0\ket{001}+\alpha_1\ket{110}$ in the $\mathcal{H}'_{01}$ subspace.
Bob's measurement
$$
  P_{01}
  = \proj{001} + \proj{101}
...
$$
  ...
  \equiv XZ\id\id Y.
$$
We say that such an $E_i$ represents an error consisting of the bit error $X$ error on the first qubit, phase error $Z$ error on the second qubit, and both bit and phase $Y$ error on the fifth qubit.^[One final time: *this is not entirely accurate if the corresponding states of the environment are not mutually orthogonal*, but it gives the right kind of intuition nonetheless.]
...
(ref:error-correction-idea-caption) Quantum error correction can be thought of as a three-step process: encoding, transmitting through a noisy channel, and then detecting and decoding.
...
Generalising this idea further is the subject of Section \@ref(the-repetition-code).
...
## The repetition code {#the-repetition-code}
...
(ref:error-probability-for-000-caption) What can happen, and with what probability, when we transmit the codeword $000$. With this scheme, we can correct up to one error, and detect up to two.

```{r error-probability-for-000,engine='tikz',fig.width=5,fig.cap='(ref:error-probability-for-000-caption)'}
\usetikzlibrary{decorations.markings,decorations.pathmorphing,decorations.pathreplacing}
\begin{tikzpicture}
...
One more important number that we need is the **distance** of such an encoding, which is defined to be the minimum number of errors that can pass undetected (or, equivalently, the minimum **Hamming distance**^[That is, the number of bit-flips required to move from one to the other. We talk about the Hamming distance more in Section \@ref(metrics).] between two codewords).
