...
In Dirac notation you can put into the box $\ket{\phantom{0}}$ anything that serves to specify what the vector is: it could be $\ket{\uparrow}$ for spin up and $\ket{\downarrow}$ for spin down (whatever this technical terminology "spin" means), or $\ket{0}$ for a quantum bit holding logical $0$ and $\ket{1}$ for a quantum bit holding logical $1$ (something that we will explain [in a bit](#qubits-gates-and-circuits)), etc.
...
Suppose that we modified the Born rule, so that probabilities were given by the absolute values of amplitudes *raised to power $p$* (for some $p$ not necessarily equal to $2$).
