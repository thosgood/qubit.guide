...
> Finally, about Shor's famous algorithm for prime factorisation.
...
but let's turn this into a black-box scenario, often known as the **binary observable measurement** problem:^[You might recognise this as Exercise \@ref(unknown-phase).]
...
- function evaluation;
- another Hadamard (or Fourier) transform.^[As explained in Section \@ref(phase-estimation-and-quantum-fourier-transforms), the Hadamard transform is a special case of the Fourier transform over the group $\mathbb{Z}_2^n$.]
...
Now we're going to generalise the binary observable measurement problem to other controlled-$U$ gates, and in the next section we will see what sort of quantum algorithm this leads us to develop; afterwards, we'll explain how this is the first step towards Shor's algorithm for prime factorisation.

Let's start by rephrasing binary observable measurement in terms of **eigenvalue estimation**.

Recall the problem: we are given the usual quantum interference circuit
...
and promised that $\varphi$ is either $0$ or $\pi$, but we are only allowed one run of the circuit to determine which one it was.

So let $U$ be a unitary and $\ket{u}$ an eigenvector.
If we know that the corresponding eigenvalue is $\pm1$ (i.e. $e^{i\varphi}$ for $\varphi\in\{0,\pi\}$) then we can simply run the phase kick-back circuit from Section \@ref(phase-kick-back) (since the first register is exactly the usual quantum interference circuit we are interested in)
...
To see that this solves our problem, we can just use the one fact that we have yet to use, namely that $\varphi\in\{0,\pi\}$, because $(\cos0,\sin0)=(1,0)$ and $(\cos\pi/2,\sin\pi/2)=(0,1)$, so if we measure $\ket{0}$ the we know that $\varphi$ must have been $0$, and similarly for $\ket{1}$ and $\varphi=\pi$.

Indeed, the fact that quantum Boolean function evaluation is given by
$$
  \ket{x}\ket{y} \longmapsto \ket{x}\ket{y\oplus f(x)}
$$
means that the unitary $U$ that it implements satisfies $U^1=\id$, since two consecutive evaluations gives
$$
  \begin{aligned}
  \ket{x}\ket{y}
...
\\=& \ket{x}\ket{y}.
  \end{aligned}
$$
So we see that the eigenvalues being $\pm1$, or, equivalently, $\varphi$ being $0$ or $\pi$, is fundamental to this story --- this is what we want to generalise, to be able to deal with more values of $\varphi$.
...
Our task is to find the best $t$-bit approximation to $m/2^n=\varphi/2\pi$.
:::

Rather than giving the algorithm that solves this problem immediately, let's work our way up to it, starting with some simple mathematical observations.
...
[you might recognise this as exercise \@ref(unknown-phase).]: 
[**exercise.** prove this!]: 
[recall the big-$o$ notation introduced in exercise \@ref(big-o).]: 
