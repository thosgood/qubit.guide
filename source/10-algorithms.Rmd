...
In other words, if you take the sum^[Even if you don't immediately see how this sum works for $z\neq a$ (writing $\ket{z}$ to mean the output), you can first calculate the probability that the output is $z=a$. In this case it is easy to see that the sum is $2^n$, and that in the final state $\sum_z\alpha_z\ket{z}$ the term $z=a$ has amplitude $1$. Thus, by normalisation, all the other terms must be equal to $0$.] over $x$, then all the terms always cancel out *unless* $a\oplus y = 00\ldots0$, but this happens *if and only if* $y=a$.
...
\begin{equation*}
  \begin{quantikz}
    ...
    & \qw
    & \ctrl{4}
      \gategroup[wires=5,steps=4,background,style={rounded corners,dashed,ultra thick,inner sep=0.5em},label style={yshift=0.4em}]{Grover iteration operator $G$\\(repeat $O(2^{n/2})$ times)}
    & \gate{H}
    & \ctrl{4}
    & \gate{H}
    & \qw
...
Here, the basic step is the **Grover iteration operator $G$**, which is the boxed part of the circuit that we repeat over and over: $(H^{\otimes n}\otimes\id)\circ U_{f_0}\circ(H^{\otimes n}\otimes\id)\circ U_f$.
After^[Recall the big-$O$ notation introduced in Exercise \@ref(big-o).] $O(2^{n/2})$ applications of $G$, we measure the first register bit-by-bit and obtain the value of $\ket{z}$, which is such that, *with "high" probability*, $f(z)=1$.
