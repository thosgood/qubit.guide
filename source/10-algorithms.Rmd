...
$$
  ...
  \end{aligned}
$$
where $f^{-1}(i)\coloneqq\{x\in\{0,1\}^n\mid f(x)=i\}$.
...
$$
  ...
  \\&= (\cos\vartheta)\ket{a} + (\sin\vartheta)\ket{b}
  \end{aligned}
$$
...
$$
to parametrise $\sqrt{\frac{N-M}{N}}$ as $\cos\vartheta$ and $\sqrt{\frac{M}{N}}$ as $\sin\vartheta$ (with $\vartheta\approx\sqrt{\frac{M}{N}}$, since $N\gg M$).

The state $\ket{s}$ is our starting input for our sequence of Grover iterations, and we will show that applying $G$, when restricting to the plane spanned by $\ket{a}$ and $\ket{b}$, amounts to applying a rotation by angle $2\vartheta$.
...
Now recall the purely geometric fact that if we have two intersecting lines $L_1$ and $L_2$ in two-dimensional Euclidean space, meeting with angle $\theta$, then reflecting an object through $L_1$ and then reflecting the resulting image through $L_2$ is the same as simply rotating the original object around the point of intersection $L_1\cap L_2$ by an angle of $2\theta$.

The angle between $\ket{a}$ and $\ket{s}$ is $\vartheta$, so each time $G$ is applied the vector is rotated (around the origin) by an angle of $2\vartheta$ towards the $\ket{b}$-axis.
...
The state $\ket{s}$ starts at angle $\vartheta$ to $\ket{a}$, and we should perform our final (and only) measurement when this angle is $\pi/2$, i.e. when $(2r+1)\vartheta = \pi/2$, which gives
$$
...
\begin{tikzpicture}[scale=1]
  \begin{scope}
    ...
    \node at (10:0.6) {\footnotesize$\vartheta$};
  ...
  \begin{scope}[shift={(4,0)}]
    ...
    \draw [->] (-1.4,0) to (1.4,0) node [right] {$\ket{a}$};
    %
    \draw [thin] (0,0) to (-20:0.7) to [bend right=1.8cm] (20:0.7) to cycle;
    \node at (-10:0.6) {\footnotesize$\vartheta$};
    \node at (10:0.6) {\footnotesize$\vartheta$};
    \draw [-Latex,primary] (0,0) to (20:1) node [right] {$\ket{s}$};
    \draw [-Latex,secondary] (0,0) to (-20:1) node [right] {$I_a\ket{s}$};
  \end{scope}
  \node at (4,-1.7) {applying $I_a$};
  \begin{scope}[shift={(8,0)}]
    ...
    \node at (-10:0.6) {\footnotesize$\vartheta$};
    \node at (10:0.6) {\footnotesize$\vartheta$};
    \node at (40:0.55) {\footnotesize$2\vartheta$};
    \draw [-Latex,primary] (0,0) to (20:1) node [right] {$\ket{s}$};
    \draw [-Latex,secondary] (0,0) to (60:1) node [above right] {$I_sI_a\ket{s}$};
    \draw [-Latex,secondary] (0,0) to (-20:1) node [right] {$I_a\ket{s}$};
  \end{scope}
  \node at (8,-1.7) {applying $G=I_sI_a$};
\end{tikzpicture}
```
...
Instead of collapsing the state to just one term in a superposition, we can express Equation ($\ddagger$) as^[Recall that the image of $f$ is the set of all $z\in\{0,1\}^n$ such that there exists some $x\in\{0,1\}^n$ satisfying $f(x)=z$.]
$$
