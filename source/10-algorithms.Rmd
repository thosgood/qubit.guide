...
In Section \@ref(deutschs-algorithm) we

TO-DO: talk (a BIT, because we'll talk about this more in the next section) about 11.4 (Deutsch) and how it relates to binary observable measurement (see 7.5 in v5)

TO-DO: this is the first step towards Shor's algorithm!

<!-- Recall Exercise \@ref(unknown-phase), where we were given the usual quantum interference circuit
...
and promised that $\varphi$ was either $0$ or $\pi$, but were only allowed one run of the circuit to determine which one it was.
This is actually a specific example of a more general problem, namely **eigenvalue estimation**.
To understand this, let's first give a solution to the above exercise.

Let $U$ be a unitary and $\ket{u}$ an eigenvector.
