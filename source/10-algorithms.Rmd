...
but let's turn this into a black-box scenario, often known as the **binary observable measurement** problem:^[You might recognise this as Exercise \@ref(unknown-phase), and we will return to this problem again in Section \@ref(phase-estimation-and-quantum-fourier-transforms).]
...
We will come back to this link between Deutsch's algorithm and binary observable measurement in Section \@ref(phase-estimation-and-quantum-fourier-transforms).
...
\begin{equation*}
  \begin{quantikz}
    ...
    & \ctrl{1}
    & \gate{H}
    & \qw \rstick{$\begin{cases}0&\mbox{if }\varphi=0\\1&\mbox{if }\varphi=\pi\end{cases}$}
  \\\lstick{$\ket{u}$}
    & \qw
    & \gate{U}
    & \qw
...
So we see that the eigenvalues being $\pm1$, or, equivalently, $\varphi$ being $0$ or $\pi$, is fundamental to this story --- this is what we want to generalise, so that we can deal with more values^[We won't be able to find the value of completely arbitrary phases, but only those of a certain rational form. Still, this is a big improvement!] of $\varphi$.
...
Our goal is to find the best $t$-bit approximation to ${\varphi}/{2\pi}={m}/{2^n}$ for some fixed $t\in\mathbb{N}$.
::::
...
We've dealt with "shrinking" the distance between the two possible phases (if we have $0$ and $\pi/k$ for some $k\in\mathbb{N}$ then we simply replace $U$ by $U^k$), so now let's look at the other way^[Any interval $[a,b]$ in $\mathbb{R}$ can be turned into any other interval $[c,d]$ by a single shrink (or scale) and a single shift: multiply $a$ and $b$ by $(d-c)/(b-a)$ to get $a'$ and $b'$, and then add $c-a'$ to both $a'$ and $b'$. Make $[a,b]$ the right length, and then just put it in the right place!] of modifying a pair of phases: shifting.
...
It's clear that "all we need to do" is add $\pi/2$ to each of the phases, since then we'd end up back at $0$ and $\pi$, but how can we actually do this?
