...
::: {.technical}
[Soon](#unitaries-as-rotations) we will explain what this group $\mathrm{SU}(2)$ is and how it relates to another important group called $\mathrm{SO}(3)$, but it turns up in many places throughout quantum physics, as well as mathematics in general.
...
While we're talking about phase, let's also justify why we keep on saying "let's ignore the global phase factors".
...
These three operators, combined with the identity, satisfy a lot of nice formal properties, which we shall examine briefly here, and then return to in more detail [later on](#pauli-matrices-algebraically).
...
The identity is just a quantum wire, and we have [already seen](#phase-gates-galore) the $X$ and $Z$ gates as the bit-flip and phase-flip (respectively).
...
Here we will just show how to represent single-qubit state vectors in terms of Euclidean vectors in three dimensions; [later](#unitaries-as-rotations) we will actually relate unitary operations on state vectors to rotations in this Euclidean space, demonstrating this isomorphism.^[That is, we have the group $\mathrm{U}(2)$ acting on the space of single-qubit state vectors, and we have the group $\mathrm{SO}(3)$ acting on the unit sphere $S^2\subset\mathbb{R}^3$. In this chapter we will discuss how to go from one _space_ (i.e. the thing being acted upon by the group) to the other; [later](#unitaries-as-rotations) we will discuss how to go from one _group_ (i.e. the thing acting on the space) to the other.]
...
We give a complete answer to this question [soon](#unitaries-as-rotations), but we might as well give some specific results here first, since some are easy enough to calculate "by hand".
...
For example, take the Hadamard gate and the $T$ gate (also known as the $\frac{\pi}{8}$-phase gate, [as we saw earlier](#phase-gates-galore)).
