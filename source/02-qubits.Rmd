...
In words, the state $\ket{k}$ evolves into the specific state $\ket{l}$ with probability amplitude $U_{lk}$ and probability $|U_{lk}|^2$, so the whole situation is described by the superposition (i.e. the sum) of all of these.
...
Pulses of light of the appropriate frequency, duration, and intensity can take the atom back and forth between the basis states $\ket{0}$ and $\ket{1}$ (implementing logical $\NOT$).
...
The gates in a circuit can be divided into layers, where the gates in the same layer operate at the same time, and the number of such layers is called the **depth** of a circuit.
:::

Some unitary $U$ acting on a single qubit is represented diagrammatically as

```{r,engine='tikz',engine.opts=list(template="latex/tikz2pdf.tex"),fig.width=1}
\begin{quantikz}
  \qw & \gate{U} & \qw
\end{quantikz}
```
...
We represent it graphically as^[We sometimes write the phase gate as $P_\varphi$ instead, if it makes the circuit easier to read.]
...
|  |  |  |
|:-|:-|:-|
**Hadamard** | $H = \frac{1}{\sqrt{2}}\begin{bmatrix}1&1\\1&-1\end{bmatrix}$ | $\begin{array}{lcr}\ket{0}&\longmapsto&\frac{1}{\sqrt{2}}(\ket{0}+\ket{1})\\\ket{1}&\longmapsto&\frac{1}{\sqrt{2}}(\ket{0}-\ket{1})\end{array}$
...
\begin{tabular}{lll}
  ...
  & $\begin{array}{lcr}\ket{0}&\longmapsto&\frac{1}{\sqrt{2}}(\ket{0}+\ket{1})\\\ket{1}&\longmapsto&\frac{1}{\sqrt{2}}(\ket{0}-\ket{1})\end{array}$
\\[2em]
  \textbf{Phase-shift:}
  & $P_\varphi = \begin{bmatrix}1&0\\0&e^{i\varphi}\end{bmatrix}$
  & $\begin{array}{lcr}\ket{0}&\longmapsto&\ket{0}\\\ket{1}&\longmapsto&e^{i\varphi}\ket{1}\end{array}$
\end{tabular}
:::
...
Let us call the resulting logic gate **the square root of $\NOT$**, or $\sqrt{\NOT}$.
...
\tikzset{operator/.style={draw,fill=white}}
  \begin{tikzpicture}[scale=1.3]
    ...
    \draw (-1.5,0) to (-1,0) to node [operator] {$\sqrt{\NOT}$} (0,0) to node [operator] {$\sqrt{\NOT}$} (1,0) to (1.5,0);
    \node at (0,-0.65) {$\NOT$};
  \end{tikzpicture}
```
...
It may seem reasonable to argue that since there is no such operation in logic, $\sqrt{\NOT}$ is impossible.
But it does exist!
Experimental physicists routinely construct such "impossible" gates in their laboratories.
It is a physically admissible operation described by the unitary matrix^[There are infinitely many unitary operations that act as the square root of $\NOT$.]
$$
  \sqrt{\NOT}
  = \frac{1}{2}
  \begin{bmatrix}
...
(ref:sqrt-not-caption) A computation that, when repeated, gives exactly $\NOT$. An unlabelled line means that it has probability $1$, and the lack of a line corresponds to having probability $0$.
...
\begin{tikzpicture}[xscale=3,yscale=2]
  ...
  \draw [thick] (i0) to node [centrelabel] {$\frac{1+i}{2}$} (1,1);
  \draw [thick] (i0) to node [pos=0.25,fill=white] {$\frac{1-i}{2}$} (1,0);
  \draw [thick] (i1) to node [pos=0.25,fill=white] {$\frac{1-i}{2}$} (1,1);
  \draw [thick] (i1) to node [centrelabel] {$\frac{1+i}{2}$} (1,0);
  \draw [thick] (m0) to node [centrelabel] {$\frac{1+i}{2}$} (o0);
  \draw [thick] (m0) to node [pos=0.25,fill=white] {$\frac{1-i}{2}$} (o1);
  \draw [thick] (m1) to node [pos=0.25,fill=white] {$\frac{1-i}{2}$} (o0);
  \draw [thick] (m1) to node [centrelabel] {$\frac{1+i}{2}$} (o1);
  %
  \node at (2.5,0.5) {$=$};
  %
  ...
  \draw [thick] (i1) to (o0);
  %
  \node at (0.5,-0.45) {$\sqrt{\NOT}$};
  \node at (1.5,-0.45) {$\sqrt{\NOT}$};
  \node at (3.5,-0.45) {\NOT};
\end{tikzpicture}
```
...
\begin{tikzpicture}
  \begin{scope}[xscale=3]
    ...
    \draw[|->] (input) to node[label=above:{$\sqrt{\NOT}$}]{} (inter);
    \draw[|->] (inter) to node[label=above:{$\sqrt{\NOT}$}]{} (output);
  \end{scope}
\end{tikzpicture}
```

Or, if you prefer to work with column vectors and matrices, you can write the two consecutive application of $\sqrt{\NOT}$ to state $\ket{0}$ as^[Just remember that circuits diagrams are read from *left to right*, and vector and matrix operations go from *right to left*.]
$$
...
One way or another, quantum theory explains the behaviour of $\sqrt{\NOT}$, and so, reassured by the physical experiments^[One such experiment (which we will soon discuss, in Section \@ref(beamsplitters-against-logic)) is the so-called Mach-Zehnder interferometer.] that corroborate this theory, logicians are now entitled to propose a new logical operation $\sqrt{\NOT}$.
Why?
...
\\[2em]
  ...
  & $\begin{array}{lcr}\ket{0}&\longmapsto&\ket{0}\\\ket{1}&\longmapsto&i\ket{1}\end{array}$
\\[2em]
  \textbf{$\frac{\pi}{8}$-phase:}
  & $T = \begin{bmatrix}1&0\\0&e^{i\frac{\pi}{4}}\end{bmatrix}$
  & $\begin{array}{lcr}\ket{0}&\longmapsto&\ket{0}\\\ket{1}&\longmapsto&e^{i\frac{\pi}{4}}\ket{1}\end{array}$
\end{tabular}
:::
...
::: {.technical title="The groups $\mathrm{SU}(2)$ and $\mathrm{SO}(3)$" latex="{The groups $\mathrm{SU}(2)$ and $\mathrm{SO}(3)$}"}
...
::: {.technical title="Global phase" latex="{Global phase}"}
...
Note that, of the $X$ and $Z$ gates, only the $X$ gate has a classical analogue (namely the logical $\NOT$ operator).
...
::: {.technical title="Parameter counting" latex="{Parameter counting}"}
...
```{r universal-circuit-for-2-by-2,engine='tikz',engine.opts=list(template="latex/tikz2pdf.tex"),fig.width=4,fig.cap='(ref:universal-circuit-for-2-by-2-caption)'}
\begin{quantikz}
...
The tacit assumption here is that we have _infinitely many_ phase gates: one gate for each phase.
...
## *Remarks and exercises* {#remarks-and-exercises-qubits}
...
2. Find the overall unitary corresponding to the circuit^[If you get the answer $\begin{bmatrix}0&1\\i&0\end{bmatrix}$ then you performed the matrix multiplication in the wrong order!]
...
$$
as a superposition of the states $\ket{\pm}=(\ket{0}\pm\ket{1})/\sqrt{2}$.
...
$$
  \begin{aligned}
    \ket{0}
    &\mapsto \alpha\ket{0} + \beta\ket{1}
  \\\ket{1}
    &\mapsto \beta^\star\ket{0} - \alpha^\star\ket{1}.
  \end{aligned}
$$
...
1. Show that this linear operator is equal to
  $$
    M
    = \big(\alpha\ket{0}+\beta\ket{1}\big)\bra{0} + \big(\beta^\star\ket{0}-\alpha^\star\ket{1}\big)\bra{1}
  $$
  by calculating $M\ket{0}$ and $M\ket{1}$.
...
Imagine that some quantum system has four energy levels: $\ket{0}$, $\ket{1}$, $\ket{2}$, and $\ket{3}$.
Consider the three orthonormal vectors
$$
  \begin{aligned}
    \ket{\psi_1}
    &= \frac{1}{\sqrt{3}}(\ket{0}+\ket{1}+\ket{2})
  \\\ket{\psi_2}
    &= \frac{1}{\sqrt{2}}(\ket{0}-\ket{2})
  \\\ket{\psi_3}
    &= \ket{3}.
  \end{aligned}
$$
Find a vector $\ket{\psi_4}$ such that $\{\ket{\psi_1},\ket{\psi_2},\ket{\psi_3},\ket{\psi_4}\}$ is an orthonormal basis.


### Some sums of inner products

Let $\{\ket{v_1},\ldots,\ket{v_N}\}$ be an orthonormal basis.
Evaluate the following:^[All of your answers should be numbers (in fact, they'll all even be integers).]

1. $\sum_{k=1}^{N-1} k^2 \braket{v_{N-1}}{v_k}$

2. $\sum_{k=1}^{N-1} k^2 \braket{v_N}{v_k}$

3. $\sum_{k=1}^N \braket{v_{N-1}}{v_k}\braket{v_k}{v_{N-1}}$

4. $\sum_{j,k=1}^{N-1} \braket{v_j}{v_k}$
...
2. Using the above, find $\theta_1$ and $\theta_2$ such that
...
3. We can think of the second identity in the previous question as an implementation of "if the two $X$ gates are absent then do nothing; otherwise implement the phase gate $P_\delta$".
...
Let $M$ be a linear operator that sends an orthonormal basis $\{\ket{u_1},\ldots,\ket{u_N}\}$ to a set of states $\{\ket{v_1},\ldots,\ket{v_n}\}$ where each $\ket{v_i}$ is of length $1$.

1. Show that the length of the vector $M(\ket{u_1}+\ket{u_2})/\sqrt{2}$ is
  $$
    \sqrt{1+\Re\braket{v_1}{v_2}}.
  $$
2. Find a correctly normalised superposition $\ket{\psi}=\lambda(\alpha\ket{u_1}+\beta\ket{u_2})$ such that $M\ket{\psi}$ is of length
  $$
    \sqrt{1+\Im\braket{v_1}{v_2}}.
  $$
3. Using the above, show that, for all $\ket{\psi}$ of length $1$, the vector $M\ket{\psi}$ is of length $1$ if and only if $M$ is unitary.
...
Let's say we want to derive the identity^[*Hint: all you need here are the Pauli matrices' commutation and anti-commutation relations, but it is instructive to derive the identity using the component notation, and below we give a sketch of how such a derivation would go.*]
$$
