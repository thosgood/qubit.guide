...
If she instead follows the above procedure, then *she* knows that it is simply described by the state vector $\ket{\psi_i}$, but the best description available to *Bob* is $\sum_i p_i\proj{\psi_i}$, as we will now justify.
...
$$
the **density operator**, since it has all the defining properties of a density operator (it is a convex sum of rank-one projectors).
