...
Let us use this as a chance to practice some of the [trace identities](#some-useful-identities).
...
We [have already mentioned](#projection-rule-and-incomplete-measurements) that non-orthogonal states cannot be reliably distinguished, and now we can make this statement more precise.
...
*(cf. [arXiv:1404.1507](https://arxiv.org/abs/1404.1507): "The attack shows that Wiesner’s scheme can only be safe if the bank replaces valid notes after validation."; also look at <https://journals.aps.org/prl/abstract/10.1103/PhysRevLett.74.4763> (and maybe <https://aapt.scitation.org/doi/pdf/10.1119/1.1445406>))*
...
To talk about this, we need to introduce the notion of **entanglement**, and this will be the subject of the [next chapter](#quantum-entanglement).
...
This is the result of the paper "Quantum Theory From Five Reasonable Axioms", by L. Hardy [arXiv:quant-ph/0101012](https://arxiv.org/abs/quant-ph/0101012).
...
Unitary operators describing evolutions of quantum systems are usually derived from the **Schrödinger equation**.^[Recall [our previous discussion](#some-quantum-dynamics) of this equation.]
$$
...
$$
  \begin{aligned}
    e^{-it\hat{H}}
    &\equiv \id - it\hat{H} + \frac{(-it)^2}{2}\hat{H}^2 + \frac{(-it)^3}{2\cdot3}\hat{H}^3 +\ldots
...
Show that the two outcomes $\pm1$ correspond to the projectors $\frac12(\id\pm S)$.
...
The optimal measurement to distinguish between the two equally likely non-orthogonal signal states, $\ket{s_1}$ and $\ket{s_2}$, is described by the two orthogonal vectors $\ket{d_1}$ and $\ket{d_2}$, placed symmetrically around the signal states, [as we have already seen](#distinguishability-of-non-orthogonal-states).
