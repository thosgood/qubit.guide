...
As we can see, if the qubit is prepared in state $\ket{\psi} = \alpha_0\ket{0} + \alpha_1\ket{1}$ and subsequently measured in the standard basis, then the outcome is $\ket{k}$ (for $k=0,1$) with probability^[This slick argument is a good example of how nice the bra-ket notation can be when we leverage the ambiguity of an expression like $\bra{a}\ket{b}\bra{b}\ket{a}$, which we can read as the scalar product of two scalars or as a projector sandwiched between a bra and a ket.]
$$
  \begin{aligned}
    |\alpha_k|^2
    &= |\braket{k}{\psi}|^2
