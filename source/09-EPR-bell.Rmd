...
This means that the expected value^[[Recall](#observables) that the expected value of an operator $E$ in the state $\ket{\phi}$ is equal to $\braket{\phi|E}{\phi}$.] of $A\otimes B$, in any state, has a simple interpretation:
$$
...
Recalling [our previous discussion on compatible operators](#compatible-observables-and-uncertainty), we know that no quantum state of a qubit can be a simultaneous eigenstate of two *non-commuting* operators, such as $\sigma_x$ and $\sigma_z$.
...
::: {.idea latex=""}
There is absolutely *no quantum theory involved* in the CSHS inequality
$$
  -2 \leq \av{S} \leq 2
$$
...
$$
  ...
  | \leq 2
$$
(which is just another way of phrasing the CSHS inequality).
:::
