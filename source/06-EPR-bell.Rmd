...
<!-- "Bell’s theorem does not disprove the existence of hidden variables, but rather disproves the existence of local hidden variables. Should they exist, they cannot be locally determined upon the preparation of the state. This in itself does not rule out the possibility that a qubit has a definite value of σ_x and σ_z at the same time. It is however ruled out by the PBR theorem." -->

<!-- mention sheaf theoretic approach that deals with both locality and contextuality in the same framework? -->
::::
...
Resolving the locality loophole puts extremely tight constraints on how quickly choices must be made, to the extent that Alice and Bob cannot make those choices manually --- they need to use random number generators.
