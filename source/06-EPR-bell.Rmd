...
But there is another approach that we can try: rather than directly trying to verify the veracity of the device, we can try to use it to turn a small amount of true randomness into a larger amount.
...
Say that Alice and Bob share a singlet state $\ket{\psi}$ that Alice then measures in the $X$ basis and Bob in the $Z$ basis, each keeping their outcomes private.
Their results are random and independent: they have generated two new truly random bits, but they only know their own, not both.
