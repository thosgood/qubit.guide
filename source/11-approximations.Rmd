...
where $S$ and $T$ are the $\pi/4$- and $\pi/8$-phase gates (Section @\ref(phase-gates-galore)), $\texttt{c-}S$ is the *controlled* $S$-gate, and $\textt{Toff}$ is the Toffoli gate (Exercise \@ref(toffoli-gate)).
...
How many circuits (acting on $n$-qubits) can we construct using $t$ gates form this set?
...
What is important is that the $g\binom{n}{k}$ term is *polynomial* in $n$, and $g$ and $k$ are fixed constants, so we will write this upper bound as
$$
  (\mathrm{poly}(n))^t.
$$
...
**TO-DO: include end of §1 in the draft**
