...
$$
  ...
  \end{aligned}
$$
by using the fact^[We have already used this property when calculating the expectation value of an observable $A$ that pertains only to a subsystem $\mathcal{A}$ of a bipartite system $\mathcal{AB}$ described by some density operator $\rho_{\mathcal{AB}}$, noting that $\tr[(A\otimes\id)\rho_{\mathcal{AB}}]=\tr[A\rho_{\mathcal{A}}]$, where $\rho_{\mathcal{A}}=\tr_\mathcal{B}\rho_{\mathcal{AB}}$.] that $(\tr\otimes\tr)[(A\otimes\id)C]=(\tr\otimes\id)[A(\id\otimes\tr)C]$, which is just another way of writing the defining property of the partial trace: $\tr_{\mathcal{AB}}(A\otimes\id)C=\tr_{\mathcal{A}}(A\tr_{\mathcal{B}}C)$.
...
::: {.idea latex=""}
**Channel--state duality.**
Quantum channels are exactly the completely positive trace-preserving (CPTP) maps, and these are exactly the linear maps $\mathcal{E}$ whose Choi matrix $\widetilde{\mathcal{E}}$ is positive semi-definite and such that $(\id\otimes\tr)\widetilde{\mathcal{E}}=\frac{1}{d}\id$.
:::
