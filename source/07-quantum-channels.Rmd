...
So, in general, we can express the evolution of the target qubit^[We can also focus on the evolution of the control qubit: see [the examples and exercises section](#control-controlled-NOT). In fact, we can choose any subset of qubits for our inputs and outputs. For example, our input could be the control qubit, and the output could be *both* the control *and* the target qubits.] as
$$
  \rho\longmapsto
...
- **Stinespring**^[[William Forrest "Woody" Stinespring](https://en.wikipedia.org/wiki/W.%5FForrest%5FStinespring) (1929--2012) was an American mathematician specialising in operator theory.] **dilation.**
    ...
    This is the approach that we discussed in [the previous section](#evolution-of-open-systems).
...
For more on this, see [the remarks on Markov approximation at the end of this chapter](#markov-approximation).
...
Show, using the [Pauli twirl](#pauli-twirl) or otherwise, that we can rewrite the depolarising channel as
$$
  \rho \longmapsto \rho'
...
A maximally mixed state of a bipartite system can be written, using the Schmidt decomposition^[As discussed in [a previous appendix](#the-schmidt-decomposition).], as
$$
  \ket{\Omega}
