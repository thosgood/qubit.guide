...
\tag{$\star$}
$$
where $\id_{AB}$ and $\id_B$ are the identity operators on $\mathcal{H}_\mathcal{A}\otimes\mathcal{H}_\mathcal{B}$ and $\mathcal{H}_\mathcal{B}$, respectively.
:::
...
here, with $\ket{k}$ fixed, it is only the $k$-th column of the block matrix $U$ that determines the evolution of $\mathcal{B}$.
For example, for $k=2$, the second block column is selected:
