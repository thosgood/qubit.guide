...
In Section \@ref(phase-estimation) we showed how to determine the eigenvalue phase of a controlled-$U$ gate, hidden by an oracle, under the assumption that the phase was of a particularly nice rational form: $2\pi m/2^n$.
...
where the inverse quantum Fourier transform is the coherent version, as in Figure \@ref(fig:example-iqft-without-measurement).

As before, the Hadamard gate followed by the controlled-$U^\triangle$ gate prepare the state
$$
  \frac{1}{\sqrt{N}} \sum_{x=0}^{N-1} e^{i\phi x}\ket{x}\ket{u}
$$
where $N=2^n$, with $n$ the number of qubits in the first register.
