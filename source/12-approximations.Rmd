...
We have already seen, all the way back in \@ref(modifying-the-born-rule)^[Think how far you've come since then!], how the Euclidean norm (from which we get the Euclidean distance) is the special case $p=2$ of $p$-norms (also known as [**$\ell^p$-norms**](https://en.wikipedia.org/wiki/Lp_space)), where
$$
...
2. Suppose that you are given one randomly selected qubit from a pair in the state from Exercise \@ref(some-density-operator-calculations)
    $$
      \ket{\psi} =
      \frac{1}{\sqrt{2}}\left(
        \ket{0}\otimes\left(
