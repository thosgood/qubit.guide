...
A vital part of the formalism of quantum theory that is missing from our above description is that of **tensor products**.
To talk about this, we need to introduce the notion of **entanglement**, and this is the subject of [Chapter 5](#chapter5), but we briefly discuss some theory, as well as physical aspects, here.

Efficiency of computation requires that we divide data into separate pieces, say physical bits.
However, a quantum description of a physical system made out of several subsystems requires a new mathematical structure called the **tensor product**.
The system as the whole can be prepared in a quantum state which does not allow
to attribute separate state vectors to its parts.
Such states are said to be **entangled**.
A quantum state of an individual subsystem must be then described by an operator called a **density operator**.
...
For large $N$ this gives an exponential gap in the energy expenditure between the binary encoding using physical bits and the so-called **unary encoding**, using energy levels of harmonic oscillators.
