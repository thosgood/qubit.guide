...
_This, however, should not be taken literally unless the states of the environment, $\ket{e_1}$ and $\ket{e_z}$, are orthogonal._^[Why not?]
...
```{r decoherence-interference,engine='tikz',fig.width=4,fig.cap='The usual interference experiment, but with decoherence.'}
...
```{r visibility-suppression,engine='tikz',engine.opts=list(template="tikz2pdf.tex"),fig.width=4,fig.cap='Visibility suppression.'}
...
\begin{tikzpicture}
  ...
  \end{axis}
  \draw [<->,thick,secondary] (0,4) to (1,4);
  \draw [dashed,thick,secondary] (1,4) to (1,1.1);
  \node at (0.5,4.4) {$\alpha$};
  %
  \draw [<->,thick,secondary] (-1,0.33*5) to (-1,0.7*5);
  \draw [dashed,thick,secondary] (-1,0.33*5) to (5,0.33*5);
  \draw [dashed,thick,secondary] (-1,0.7*5) to (5,0.7*5);
  \node at (-1.3,2.6) {$v$};
\end{tikzpicture}
```

**!!!TODO!!! be more descriptive with the axis labelling: what is $p$ the probability of exactly?**
