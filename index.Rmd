...
To understand these we need to switch from our **rectangular coordinates** $z=x+iy$ to **polar coordinates** --- instead of describing a point $z$ in $\mathbb{R}^2$ as "$x$ units left/right and $y$ units up/down", we describe it as "$r$ units from the origin, at an angle of $\theta$".
...
**TO-DO: basically planar for plus/minus and polar for times/divide**

|                    | rectangular/planar                        | polar/Euler                                                   | geometric                               |
|--------------------|-------------------------------------------|---------------------------------------------------------------|-----------------------------------------|
| **addition**       | $(x,y)+(x',y')=(x+x',y+y')$               | ???                                                           | vector addition                         |
| **multiplication** | $(x,y)\cdot(x',y')=(xx'-yy',xy'+x'y)$     | $re^{i\theta}\cdot r'e^{i\theta'}=(rr')e^{i(\theta+\theta')}$ | scaling by $r$ and rotation by $\theta$ |
| **inversion**      | $(x,y)\mapsto (x/(x^2+y^2),-y/(x^2+y^2))$ | $re^{i\theta}\mapsto (1/r)e^{-i\theta}$                       | conjugation and scaling by $1/r^2$      |
| **conjugation**    | $(x,y)\mapsto (x,-y)$                     | $re^{i\theta}\mapsto re^{-i\theta}$                           | reflection in the real-axis             |
