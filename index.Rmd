...
We have just gone over all the linear algebra that we will need, so now let's tackle the other topic.

Probability theory is a vast and beautiful subject which has undergone many transformations over the centuries.
What started as something understood in terms of gambling odds later evolved into the theory of measure spaces, and is now able to be expressed in terms of diagrammatic category theory.
...
::: {.idea latex=""}
The **probability** $P(A)$ of an event $A$ is the number of desired outcomes divided by the number of total outcomes, i.e.
$$
  P(A)
  = \frac{\text{\# of elements of }A}{\text{\# of elements of }\Omega}.
$$

In particular, the probability will thus always be a number between $0$ and $1$.
:::
...
- coins can't remember how many times they've been flipped!
- fair = all outcomes equally likely
- mutually exclusive
- mutually exclusive events add (coins)
- consecutive independent(?) events multiply (coins)
- conditional probabilities by hand (coins)
- independent events
- Bayes' rule
- .technical: pascal's triangle
- .technical: measure theory
- .technical: diagrammatic probability theory
