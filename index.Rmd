...
**TO-DO: complex numbers are in basically every area of modern mathematics (and physics as well) so they deserve a nice little introduction**
...
We call the result --- these numbers $0,1,2,\ldots$ and the ability to add them (with $0$ being an additive identity, i.e. such that adding it to any number does absolutely nothing) --- the **monoid** of **natural numbers**, and we denote it by $\mathbb{N}$.
...
One step up from addition on the arithmetic ladder is *multiplication*, but we can already multiply things in $\mathbb{Z}$ since the product of any two integers is always just another integer.^[Mathematicians usually prefer to write multiplication as $n\cdot m$ instead of $n\times m$, and we'll use this convention too.]
...
Well, if we think of multiplication as repeated addition ("$5\cdot3$ is just $5+5+5$"), then we can ask what repeated multiplication looks like, and the answer is *exponentiation*: "$5^3$ is just $5\cdot5\cdot5$".

**TO-DO: somehow complex numbers are magic, in that, once we've closed under (integer) exponentiation, we actually get basically everything (but *don't* go into the details of tetration etc, just say "they're algebraically closed")**

**TO-DO: the real power of complex numbers (for our purposes) comes from how much *geometric* structure they have**
...
::: {.technical}
TO-DO: abelian group over a field; see also [this exercise](#complex-numbers-exercise).
:::
