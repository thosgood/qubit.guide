...
$$
  \begin{gathered}
    ...
    \right]
  \\[1em]
    =\proj{00}00\otimes\id
  \\+ \proj{01}01\otimes\id
  \\+ \proj{10}10\otimes\id
  \\+ \proj{11}11\otimes X.
  \end{gathered}
$$
...
That is, _the phase kickback mechanism introduced a relative phase in the equally-weighted superposition of all binary strings of length two_.
...
2. Using the block matrix form of $A\otimes B$ expressed in terms of $A_{ij}$ and $B_{ij}$ described above, explain how the following operations are performed on the block matrix:
...
$$
Moreover, for each particular joint state $\ket{\psi}$, we can find orthonormal bases, $\{\ket{\widetilde{a_i}}\}$ in $\mathcal{H}_A$ and $\{\ket{\widetilde{b_j}}\}$ in $\mathcal{H}_B$, such that $\ket{\psi}$ can be expressed as
$$
  \ket{\psi}
...
Indeed, given two bipartite states $\ket{\psi}$ and $\ket{\phi}$, we usually _cannot_ perform the Schmidt decomposition using the _same_ orthonormal bases in $\mathcal{H}_A$ and $\mathcal{H}_B$.
