...
```{r,echo=FALSE,engine='tikz',fig.width=2,fig.align='center',fig.ext=if (knitr:::is_latex_output()) 'pdf' else 'png'}
\begin{tikzpicture}
  \draw (0,0) circle (1cm);
\end{tikzpicture}
```
...
3. Last but not least, the probability of any event can be calculated using a deceptively simple rule --- the _additivity axiom_:
