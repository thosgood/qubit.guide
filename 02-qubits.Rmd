...
```{r universal-circuit-for-2-by-2,engine='tikz',fig.width=5,fig.cap='(ref:universal-circuit-for-2-by-2-caption)'}
\tikzset{operator/.style={draw,fill=white}}
\tikzset{dot/.style={fill,shape=circle,minimum size=5pt,inner sep=0pt}}
...
1. Consider the usual quantum interference circuit
    ...
    \newcommand{\ket}[1]{|#1\rangle}
    \begin{tikzpicture}[xscale=2]
      \node (input) at (-1,0) {$\ket{0}$};
      \node (output) at (1,0) {$\ket{1}$};
      \draw (input) to node [operator] {$H$} (0,0);
      \draw (0,0) to node [operator] {$H$} (output);
