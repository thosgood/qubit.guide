...
$$
  \begin{aligned}
    \rho^{AB}
    &\mapsto
    \underbrace{\rho^\mathcal{A}=\tr_\mathcal{B}\rho^{AB}}_{\mathrm{partial\,trace\,over}\,\mathcal{B}}\qquad
  \\\rho^{AB}
    &\mapsto
    \underbrace{\rho^\mathcal{B}=\tr_\mathcal{A}\rho^{AB}}_{\mathrm{partial\,trace\,over}\,\mathcal{A}}
  \end{aligned}
$$
...
When constructing $\rho^A$ we had better make sure that for any observable $X$ the average value of $X$ in the state $\rho^\mathcal{A}$ is the same as the average value of $X\otimes\id$ in the state $\rho^{AB}$.
This is indeed the case for the partial trace.

For example, let us go back to the state in \@ref(eq:bipartite-density) and assume that Alice measures some observable $X$ on her part of the system.
...
$$
  ...
  \\&= \tr X \underbrace{\sum_i p_i\proj{\psi_i}}_{\rho^A = \tr_\mathcal{B}\rho^{AB}}
  \\&= \tr X\rho^A,
  \end{aligned}
$$
as required.
...
2. Show that purification is unique up to unitary equivalence.
    Let $\ket{\psi_1}$ and $\ket{\psi_2}$ in $\mathcal{H}_A\otimes\mathcal{H}_B$ be two pure states such that $\tr_\mathcal{B}\proj{\psi_1} = \tr_\mathcal{B}\proj{\psi_2}$.
